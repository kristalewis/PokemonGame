...
<div class="battleDiv">
	<c:choose>
		...
		<c:when test="${sessionScope.battle.pokemonAttacking.trained}">
			...
			<c:url value="/fightbattle" var="battleFormUrl"/>
		...
		<c:otherwise>
			...
			<form action="${battleFormUrl}" method="POST">
				...
				<button type="submit" class="btn btn-secondary battleButton">${sessionScope.battle.pokemonAttacking.name}'s Move</button>
			</form>
		</c:otherwise>
	</c:choose>
</div>
